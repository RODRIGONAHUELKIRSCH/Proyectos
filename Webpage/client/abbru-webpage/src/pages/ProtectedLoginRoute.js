...
const ProtectedLoginRoute = ({ children }) => {
    ...
    if (!encryptedToken) {
        ...
        return _jsx(_Fragment, { children: children }); // Si no hay token, redirigir a inicio
    }
    ...
    if (decryptedToken && !isTokenExpired(decryptedToken)) {
        ...
        return _jsx(Navigate, { to: "/frontend/" }); // Si el token es válido, redirige
    }
    //console.log("El token ha expirado o no es válido. Renderizando los hijos.");
    return _jsx(_Fragment, { children: children }); // Renderiza los hijos si el token no es válido
};
