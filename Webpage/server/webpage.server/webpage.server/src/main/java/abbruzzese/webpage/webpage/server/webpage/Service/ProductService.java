...
public class ProductService {
    ...
    public DTOProduct updateProduct(String id, DTOProduct productDTO) {
        ...
        if (existingProduct.isPresent()) {
            ...
            product.setFecha(productDTO.getFecha());
    ...
    @Transactional
    public Optional<DTOProduct> getProductByName(String Nombre){
        return prep.findByNombre(Nombre)
               .map(pmap::ProducttoDTO);
    }

    @Transactional
    public List<String> getProductsByCategoria(String categoria){
        List<Product> products= prep.FindByCategoria(categoria);
        return products.stream()
                        .map(Product::getImageproduct)
                        .collect(Collectors.toList());
    }
